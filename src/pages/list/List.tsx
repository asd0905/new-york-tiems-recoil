...
import { UseInfiniteFetchNews } from "../../hooks/useInfiniteFetchNews";

const List = () => {
	const { isFetching, hasNextPage, fetchNextPage, error, status } =
		UseInfiniteFetchNews();
	const newsList = useRecoilValue(newsListAtom);
	const newParams = useRecoilValue(newsParamsAtom);
	...
	return (
		<SLayout>
						...
						{newsList?.map((news: any, index: number) => (
							<div className='newsList' key={`${news?.created_date}_${index}`}>
								...
								<div className='newsInfo'>
									...
									<span className='date'>
										{moment(news?.pub_date).format("YYYY.MM.DD dd")}
									</span>
								</div>
							</div>
						))}
			...
			{isFetching && (
				<div style={{ height: "50px", backgroundColor: "#ddd" }}></div>
			)}
		</SLayout>
	);
};
